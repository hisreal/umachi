...
<main class="clock-in-page attendance-module-page">
    ...
    <section class="container-fluid clock-workspace">
        <form class="app-card card attendance-settings-form needs-validation" id="attendanceSettingsForm" method="post" action="<?php echo e(route_url('admin/attendance-settings/update')); ?>" novalidate>
            <?php echo csrf_field(); ?>
            <div class="app-card__header"><div><span class="eyebrow">Rules</span><h2>Attendance Configuration</h2></div></div>
            <div class="row g-3">
               
                <div class="col-md-4"><label class="form-label" for="gracePeriod">Grace Period (Minutes)</label><input class="form-control" type="number" id="gracePeriod" name="grace_period" value="<?php echo e((string) $attendanceSettings['grace_period']); ?>" min="0" required><small>Employees arriving within this period are still considered present.</small></div>
                <div class="col-md-4"><label class="form-label" for="lateThreshold">Late Threshold (Minutes)</label><input class="form-control" type="number" id="lateThreshold" name="late_threshold" value="<?php echo e((string) $attendanceSettings['late_threshold']); ?>" min="0" required><small>Employees arriving after this time are marked late.</small></div>
                <div class="col-md-4"><label class="form-label" for="maxOvertime">Maximum Overtime (Hours)</label><input class="form-control" type="number" id="maxOvertime" name="max_overtime" value="<?php echo e((string) $attendanceSettings['max_overtime']); ?>" min="0" required></div>
                <div class="col-md-6"><label class="form-label" for="shiftDuration">Shift Duration</label><select class="form-select" id="shiftDuration" name="shift_duration" required><?php foreach (['8 Hours', '10 Hours', '12 Hours'] as $duration): ?><option value="<?php echo e($duration); ?>" <?php echo $attendanceSettings['shift_duration'] === $duration ? 'selected' : ''; ?>><?php echo e($duration); ?></option><?php endforeach; ?></select></div>
                <div class="col-md-6"><label class="form-label" for="approvalRequired">Attendance Approval Required</label><select class="form-select" id="approvalRequired" name="approval_required" required><?php foreach (['No Approval', 'Supervisor', 'Manager', 'Admin'] as $approval): ?><option value="<?php echo e($approval); ?>" <?php echo $attendanceSettings['approval_required'] === $approval ? 'selected' : ''; ?>><?php echo e($approval); ?></option><?php endforeach; ?></select></div>
            </div>
            <div class="attendance-switch-grid mt-4">
                <?php foreach ([['auto_clock_out', 'Auto Clock-Out'], ['photo_required', 'Require Clock-In Photo'], ['face_verification', 'Require Face Verification'], ['gps_verification', 'Require GPS Verification'], ['early_clock_in', 'Allow Early Clock-In'], ['manual_adjustment', 'Allow Manual Attendance Adjustment']] as $switch): ?>
                    <label class="attendance-switch-card" for="<?php echo e($switch[0]); ?>"><span><?php echo e($switch[1]); ?></span><div class="form-check form-switch"><input class="form-check-input" type="checkbox" role="switch" id="<?php echo e($switch[0]); ?>" name="<?php echo e($switch[0]); ?>" value="1" <?php echo !empty($attendanceSettings[$switch[0]]) ? 'checked' : ''; ?>></div></label>
                <?php endforeach; ?>
            </div>
